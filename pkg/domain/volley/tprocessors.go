...
func NewBaseStateProvider(state telegram.State, msg telegram.Message, person person.Person, loc location.Location, repository Repository, text string) (sp BaseStateProvider, err error) {
	...
	if repository != nil && state.Data != "" {
		id, err := Volley{}.IdFromBase64(state.Data)
		if err != nil {
			return sp, err
		}
...
func (p *BaseStateProvider) GetMR() (mr *telegram.MessageRequest) {
	...
	rview := NewTelegramViewRu(p.reserve)
	mtxt := rview.GetText()
